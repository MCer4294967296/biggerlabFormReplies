...
def parseBiggerlabCourseFeedback(blob):
    ...
    info["projectScreenshot"] = shorten(deDownload(blob["field_18"][0])) # 项目作品截图
    ...
    info["awardEvidence"] = shorten(deDownload(blob["field_40"][0])) # 竞赛获奖证明截图
    info["competitionName"] = blob["field_34"] # 比赛名称
    info["prizeReceived"] = blob["field_35"] # 获得的奖项

    info["projectPublishEvidence"] = shorten(deDownload(blob["field_47"][0])) # 项目发布证明截图
    info["copyrightEvidence"] = shorten(deDownload(blob["field_48"][0])) # 著作权证明截图
    info["projectLink"] = blob["field_43"] # 项目链接
    info["sourceCode"] = shorten(deDownload(blob["field_36"][0])) # 源代码文件

    info["admissionEvidence"] = shorten(deDownload(blob["field_49"][0])) # 名校录取证明截图
    info["schoolName"] = blob["field_44"] # 学校名称
    info["profession"] = blob["field_45"] # 专业名称

    info["awardOthers"] = shorten(deDownload(blob["field_50"][0])) # 其他荣誉证明截图
    return info
